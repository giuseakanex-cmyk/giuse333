...
export default function (m, conn = { user: {} }) {
  ...
  try {
    ...
    // Recupero Nomi Sincrono con aggiornamento in background asincrono
    const getNameSync = (jid) => {
      ...
      if (!cached) {
        ...
        if (!cached) {
          // Esegue in background asincrono senza bloccare il thread principale
          conn.getName(jid).then(name => {
            if (name) nameCache.set(jid, name);
          }).catch(() => {});
          return jid.split('@')[0];
        }
      }
    ...
    const chat = getNameSync(chatJid) || 'Chat Sconosciuta';
    ...
    // Controllo Admin Sincrono via Cache
    let isAdmin = false;
    let participantCount = '?';
    if (isGroup) {
      let groupMeta = groupMetaCache.get(chatJid);
      if (!groupMeta) {
        conn.groupMetadata(chatJid).then(meta => {
          if (meta) groupMetaCache.set(chatJid, meta);
        }).catch(() => {});
      } else {
        participantCount = groupMeta.participants?.length || '?';
        isAdmin = groupMeta.participants?.some(p =>
          (conn.decodeJid(p.id) === senderJid || p.jid === senderJid) &&
    ...
    // Formattazione e Stampa Sincrona del Testo
    formatTextSync(m, c, getNameSync);

    // Stampa parametri di sistema (es: utenti rimossi, aggiunti, ecc.)
    ...
    // Ripristinato il logger dei dettagli specifici dei messaggi
...
function formatPhoneNumber(jid, name) {
    ...
    if (phoneCache.has(userPart)) {
      return phoneCache.get(userPart) + (name ? ` ~${name}` : '');
    }
    try {
...
((?
  const mdFormat = (_, type, txt, code, inlineCode) => {
    const types = { '_': 'italic', '*': 'bold', '~': 'strikethrough', '`': 'gray' };
    const val = txt || code || inlineCode;
    return types[type] ? chalk[types[type]](val) : val;
  };

  if (text.length < 512) {
    text = text.replace(urlRegex, url => chalk.cyanBright(url));
  }
  text = text.replace(mdRegex, mdFormat);

  if (m.mentionedJid?.length) {
    for (const id of m.mentionedJid) {
      const originalNum = id.split('@')[0];
      const name = getNameSync(id) || originalNum;
      text = text.split('@' + originalNum).join(chalk.blueBright('@' + originalNum + (name !== originalNum ? ' ~' + name : '')));
    }
  }
  ...
  if (m.error) console.log(chalk.red(text))
  else if (m.isCommand) console.log(chalk.bgHex('#2a2a00').hex('#ffd60a').bold(` ${text} `))
  else if (m.quoted) console.log(chalk.hex('#c1a0ffff')(text))
  else console.log(chalk.whiteBright(text))
}
