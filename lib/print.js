...
export default async function (m, conn = { user: {} }) {
  ...
  try {
    ...
    const getName = async (jid) => {
      ...
      if (jid.endsWith('@newsletter')) {
        ...
        if (meta?.subject) {
          ...
          try {
            const fetchedMeta = await conn.groupMetadata(jid);
            if (fetchedMeta?.subject) {
              fetchedMeta.fetchTime = Date.now();
              groupMetaCache.set(jid, fetchedMeta);
              cached = fetchedMeta.subject;
            }
          } catch {
          }
        }
    ...
    const senderNamePromise = getName(senderJid);
    const chatNamePromise = isGroup ? getName(chatJid) : senderNamePromise;
    const [_name, chatName] = await Promise.all([senderNamePromise, chatNamePromise]);
    const sender = formatPhoneNumber(senderJid, _name);
    const chat = chatName || (_name || 'Chat Sconosciuta');
...
async function formatText(m, conn) {
  ...
  if (text.length > 4096) return chalk.whiteBright(text.trim() || '[Messaggio Vuoto]');
