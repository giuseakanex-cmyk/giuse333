...
TEL;type=CELL;type=VOICE;waid=${number}:${PhoneNumber('+' + number).getNumber('international')}${biz.description ? `
          X-WA-BIZ-NAME:${(conn.chats[njid]?.vname || conn.getCachedNameSync(njid, name) || name).replace(/\n/, '\\n')}
...
sendList: {
      ...
      async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
        const msg = proto.Message.fromObject({
          groupInviteMessage: proto.GroupInviteMessage.fromObject({
            ...
            groupJid: jid,
            groupName: (groupName ? groupName : await conn.getCachedName(jid)) || null,
  ...
  // Simple name cache to avoid repeated network calls
  conn._nameCache = conn._nameCache || new Map();
  conn.getCachedName = async function (jid) {
    if (!jid) return '';
    const key = jid.toString();
    if (conn._nameCache.has(key)) return conn._nameCache.get(key);
    try {
      const name = await conn.getName(jid);
      if (name) conn._nameCache.set(key, name);
      return name || '';
    } catch {
      return '';
    }
  };
  conn.getCachedNameSync = function (jid, fallback = '') {
    if (!jid) return fallback;
    const key = jid.toString();
    return conn._nameCache.has(key) ? conn._nameCache.get(key) : fallback;
  };
  // Helper rapido e robusto per estrarre il testo da un messaggio evitando undefined
  conn.getMessageText = function (m) {
    if (!m) return '';
    try {
  ...
  // Estrae comando e corpo in modo sicuro. Restituisce {prefix, command, body} o null
  conn.getCommand = function (m, prefixes = ['.', '!', '/', '#']) {
    const text = conn.getMessageText(m);
    if (!text) return null;
    if (text.length > 8192) return null; // evita parsing pesante su messaggi enormi
    const first = text.trim().split(/\s+/)[0];
    for (const p of prefixes) {
      if (first.startsWith(p)) {
        const without = text.slice(p.length).trim();
        const parts = without.split(/\s+/);
        const command = parts.shift() || '';
        const body = parts.join(' ');
        return { prefix: p, command, body };
      }
    }
    return null;
  };
...
export function smsg(conn, m) {
  ...
  // Cache rapido del testo e parsing comandi per evitare ripetizioni e undefined
  try {
    const txt = conn.getMessageText(m) || '';
    if (typeof txt === 'string' && txt.trim()) m._text = txt;
    const cmd = conn.getCommand(m);
    if (cmd) {
      m.isCommand = true;
      m.command = cmd.command;
      m.commandPrefix = cmd.prefix;
      m.commandBody = cmd.body;
    } else {
      m.isCommand = false;
    }
  } catch (e) {
    // Non fatto: se conn non ha helper, ignoriamo
  }
  return m;
}
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {text: q} : q)), {
          ...
          name: {
            get() {
                ...
                const cached = sender ? self.conn?.getCachedNameSync(sender, null) : null;
                if (!cached && sender) {
                  // Populate cache asynchronously for next time
                  try { self.conn?.getCachedName(sender).catch(()=>{}) } catch {}
                }
                return cached;
            },
            enumerable: true,
        ...
        name: {
            get() {
                const push = !nullish(this.pushName) && this.pushName;
                if (push) return push;
                const cached = this.conn?.getCachedNameSync(this.sender, null);
                if (!cached && this.sender) {
                  try { this.conn?.getCachedName(this.sender).catch(()=>{}); } catch {}
                }
                return cached;
            },
            enumerable: true
