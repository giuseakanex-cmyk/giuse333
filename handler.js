...
if (!global.cacheListenersSet && global.conn) {
    ...
    conn.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
            try {
                ...
                if (!global.db.data.chats[chatId]) await registerGroup(chatId)
            } catch (e) {}
        }
    })
    ...
    setInterval(async () => {
        try {
            if (!global.db.data || !conn.user || !conn.user.jid) return
            if (global.store?.messages) {
                const maxMessages = 100
                for (let jid in global.store.messages) {
                    const msgs = global.store.messages[jid]
                    if (msgs && msgs.length > maxMessages) {
                        global.store.messages[jid] = msgs.slice(-maxMessages)
                    }
                }
            }
        } catch (e) {}
...
if (user.banned) {
    ...
    continue
        ...
        if (m.isGroup && !m.fromMe) {
            chat.totalmsg = (chat.totalmsg || 0) + 1
            chat.todayMsg = (chat.todayMsg || 0) + 1
            chat.todayUsers = chat.todayUsers || {}
            chat.todayUsers[m.sender] = (chat.todayUsers[m.sender] || 0) + 1
            chat.weekMsg = (chat.weekMsg || 0) + 1
            chat.weekUsers = chat.weekUsers || {}
            chat.weekUsers[m.sender] = (chat.weekUsers[m.sender] || 0) + 1

            const weekday = new Date().toLocaleDateString('it-IT', { weekday: 'long' })
            chat.giornoCounter = chat.giornoCounter || {}
            chat.giornoCounter[weekday] = (chat.giornoCounter[weekday] || 0) + 1
            chat.giornoPiuAttivo = Object.keys(chat.giornoCounter).reduce((a, b) =>
                chat.giornoCounter[a] > chat.giornoCounter[b] ? a : b)

            chat.topUsers = chat.topUsers || {}
            chat.topUsers[m.sender] = (chat.topUsers[m.sender] || 0) + 1
            const topUsers = Object.entries(chat.topUsers)
                .filter(([jid]) => jid !== this.user.jid && !jid.endsWith('@g.us'))
                .sort((a, b) => b[1] - a[1])
...
if (chat.isBanned && !isOwner) return
        ...
        if (settings.antiPrivate && !m.isGroup && !m.fromMe && !isOwner) continue
