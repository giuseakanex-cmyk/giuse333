let handler = async (m, { conn }) => {
  ...
  if (uSender.bank < 1000)
    throw "🏦 𝐃𝐞𝐯𝐢 𝐚𝐯𝐞𝐫𝐞 𝐚𝐥𝐦𝐞𝐧𝐨 *1000€* 𝐢𝐧 𝐛𝐚𝐧𝐜𝐚 𝐩𝐞𝐫 𝐟𝐚𝐫𝐞 𝐮𝐧𝐚 𝐫𝐚𝐩𝐢𝐧𝐚!"

  if (uVictim.money <= 0) {
    let multa = Math.floor(Math.random() * 60) + 40

    uSender.bank = Math.max(0, uSender.bank - multa)
    uSender.warn = (uSender.warn || 0) + 1

    return conn.reply(
      m.chat,
      `🚨 @${who.split('@')[0]} 𝐧𝐨𝐧 𝐡𝐚 𝐬𝐨𝐥𝐝𝐢!\n𝐒𝐞𝐢 𝐬𝐭𝐚𝐭𝐨 𝐦𝐮𝐥𝐭𝐚𝐭𝐨 𝐝𝐢 *${multa}€* (𝐩𝐫𝐞𝐬𝐢 𝐝𝐚𝐥𝐥𝐚 𝐛𝐚𝐧𝐜𝐚)`,
      null,
      { mentions: [who] }
    )
  }
  ...
  if (fallisce) {
    ...
    testo = `🚨 𝐒𝐞𝐢 𝐬𝐭𝐚𝐭𝐨 𝐬𝐜𝐨𝐩𝐞𝐫𝐭𝐨!\n𝐋𝐚 𝐩𝐨𝐥𝐢𝐳𝐢𝐚 𝐭𝐢 𝐡𝐚 𝐦𝐮𝐥𝐭𝐚𝐭𝐨 𝐝𝐢 *${multa}€* (𝐩𝐫𝐞𝐬𝐢 𝐝𝐚𝐥𝐥𝐚 𝐛𝐚𝐧𝐜𝐚)`
  } else {
